...
def generate(img_size, model_dir, output_dir, timesteps, device, save_interval):
    ...
    with torch.no_grad():
        ...
        for step, img in diffusion.p_sample_loop(
            model, shape, save_interval=save_interval
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timesteps", type=int, default=TIMESTEPS, help="Number of timesteps"
    )
    parser.add_argument(
        "--device", type=str, default=DEVICE, help="Device (cpu or cuda)"
    )
    parser.add_argument(
        "--save_interval", type=int, default=50, help="Interval to save images"
    )
